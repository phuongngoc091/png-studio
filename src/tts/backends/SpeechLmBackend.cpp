...
void SpeechLmBackend::setProgressCallback(std::function<bool(int current,
                                                              int total,
                                                              const QString &stage,
                                                              int chunkIndex,
...
void SpeechLmBackend::handleProgress(const vieneu_progress *progress, void *userData)
{
    auto *self = static_cast<SpeechLmBackend *>(userData);
...
void SpeechLmBackend::cancelProcessing()
{
...
bool SpeechLmBackend::load(const QVariantMap &config, QString &error, QVariantList &schema)
{
    ...
    if (!speakerChoices.isEmpty()) {
        ...
        if (resolvedVoicesPath.isEmpty()) {
            Logger::warning("SpeechLmBackend", "SpeechLM preset voices unavailable: no voices file was provided or found next to the model.");
        } else {
    ...
    if (m_useVieneuRuntime) {
        ...
        if (!m_context) {
            ...
            Logger::error("SpeechLmBackend", "Failed to initialize VieNeu TTS model: " + error);
            unload();
            return false;
        }
        if (vieneu.vieneu_set_progress_callback) {
            vieneu.vieneu_set_progress_callback((struct vieneu_context*)m_context, &SpeechLmBackend::handleProgress, this);
        }

        if (!runtimePath.isEmpty())
            s_sessionSpeechLmRuntimePath = runtimePath;

        Logger::info("SpeechLmBackend", QString("VieNeu TTS profile loaded successfully"));
        return true;
    }
    ...
    if (m_context) {
        if (m_useVieneuRuntime) {
            ...
            if (vieneu.isLoaded() && vieneu.vieneu_free) {
                vieneu.vieneu_free((struct vieneu_context*)m_context);
            }
        } else {
            auto& slm = SpeechLmTtsInterface::instance();
                               ...
                               QVector<float> &samples, int &sampleRate, QString &error)
{
    ...
    if (m_useVieneuRuntime) {
        ...
        Logger::info("SpeechLmBackend", QString("VieNeu TTS synthesize returned: status=%1, samples=%2, sampleRate=%3")
                     .arg(status)
                     .arg(out.n_samples)
                     .arg(out.sample_rate));
        ...
        if (status != 0) {
            error = QString::fromUtf8(vieneu.vieneu_last_error());
            Logger::error("SpeechLmBackend", "VieNeu TTS synthesis failed: " + error);
            return false;
        }

        if (!isValidRuntimeAudio(out)) {
            Logger::error("SpeechLmBackend", QString("VieNeu TTS returned invalid audio buffer: samples=%1, sampleRate=%2, ptr=%3")
                          .arg(out.n_samples)
                          .arg(out.sample_rate)
                               ...
                               QVector<float> &samples, int &sampleRate, QString &error)
{
    ...
    if (m_useVieneuRuntime) {
        ...
        if (rc != 0) {
            error = QString::fromUtf8(vieneu.vieneu_last_error());
            Logger::error("SpeechLmBackend", "Failed to encode VieNeu reference audio: " + error);
            return false;
        }
        ...
        if (status != 0) {
            error = QString::fromUtf8(vieneu.vieneu_last_error());
            Logger::error("SpeechLmBackend", "Failed to synthesize VieNeu cloned voice: " + error);
            return false;
        }
        if (!isValidRuntimeAudio(out)) {
            vieneu.vieneu_audio_free(&out);
        ...
        sampleRate = out.sample_rate;
        vieneu.vieneu_audio_free(&out);
        Logger::info("SpeechLmBackend", QString("VieNeu TTS voice cloning successful"));
        return true;
    }
